...
class Client():
    ...
    def setup_connection(self):
        try:
            ...
            raise RuntimeError('Error: Could not create socket...') from emsg
        ...
        while retries > 0:
            ...
            try:
                ...
                self.shutdown()
                raise RuntimeError('Error sending init message to TORCS.') from emsg
            ...
            try:
                ...
                self.shutdown()
                raise RuntimeError('Error receiving init response from TORCS.') from emsg
        ...
        self.shutdown()
        raise TimeoutError(f"Could not connect to TORCS server on port {self.port}.")
        ...
        while True:
            ...
            if '***identified***' in sockdata:
                ...
                print((("Server has stopped the race on %d. "+
                        "You were in %s place.") %
                        (self.port,self.S.d.get('racePos', '?'))))
                self.shutdown()
                return
    ...
    def respond_to_server(self):
        ...
        try:
            ...
            print(f"Error sending to server: {emsg}")
            self.shutdown()
            return
